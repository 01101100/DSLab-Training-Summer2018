    ...
    def collect_data_from(parent_dir, newsgroup_list):
        ...
        for group_id, newsgroup in enumerate(newsgroup_list):
            ...
            for filename, file_path in files:
                with open(file_path, 'r', errors='ignore') as opening_file:
                    text = opening_file.read().lower()
...
def generate_vocabulary(data_path):
    ...
    # words_idfs.sort(key=lambda (word, idf): -idf) #FIXME: tuple unpacking is not supported in python 3
...
def get_tf_idf(data_path):
    ...
    with open('datasets/20news-bydate/words_tf_idfs.txt', 'w') as f:
        f.write('\n'.join([str(label) + "<fff>" + str(doc_id) + "<fff>" + spares_rep for label, doc_id, spares_rep in data_tf_idf]))

gather_20newgroups_data()
